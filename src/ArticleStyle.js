...
export const articlestyle = {
    ...
    h3: ({ ...props }) => <h2 style={{ color: "white", marginLeft: "8px" }}>{props.children}</h2>,
    p: ({ ...props }) => <p style={{ marginLeft: "12px" }}>{props.children}</p>,
    code: ({ ...props }) => <p style={{ marginLeft: "12px",backgroundColor: "rgba(128,128,128,0.5)",border: "1px solid gray" }}>{props.children}</p>,
    img: ({ ...props }) => {
        let style = {
            width: "128px",
            height: "128px"
